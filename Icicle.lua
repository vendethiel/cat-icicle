...
local IcicleReset = {
	...
	[14185] = {"Sprint", "Vanish", "Shadowstep", "Kick", "Dismantle", "Smoke Bomb", "Kick", "Dismantle"},  --with prep glyph "Kick", "Dismantle", "Smoke Bomb"
...
local addicons = function(name, f)
	...
	if num * iconsize + (num * 2 - 2) > width then
		-- V: let them stack up a bit more so that I can read cooldowns
		size = iconsize / 1.2
		Sfont = ceil(size / 2.5)
	else 
		size = iconsize
